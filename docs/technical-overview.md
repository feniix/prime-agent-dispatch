# Prime Agent Dispatch technical overview
...
- **Beta Milestone 1:** **COMPLETE for the disposable-fixture CLI scope, with reviewed limitations.** It productionizes the broker seam, adds immutable confirmation and trusted host policy, enforces one global job and finite budgets, and validates real Prime completion and cancellation.
- **Prime runtime integrity:** Prime 0.8.0 now launches only from a self-contained platform/architecture/Node-specific artifact. The builder proves packaged source against the pinned official release, records the pnpm lockfile, normalizes internal dependency links, produces a canonical per-entry manifest, and reproducibly packages the complete runtime. Workers privately copy, checksum, preflight, extract, verify, and atomically cache that artifact before version/help or RPC startup. See [Prime runtime artifacts](prime-runtime-artifacts.md).
...
- **Beta Milestone 2:** **IMPLEMENTED for the operational Discord fixture path.** The package in [`openclaw-plugin`](../openclaw-plugin/README.md) exposes owner-only typed tools, hash-bound one-time confirmation, editable status-card delivery, terminal notification catch-up, and the standalone CLI/control boundary. See the [Beta Milestone 2 report](beta-milestone-2.md).
- **OpenClaw deployment:** native online and offline plugin archives install with `openclaw plugins install <artifact.tgz>`. The plugin derives the active profile, manages its runtime/state internally, and starts with a fail-closed empty repository policy. The earlier source lifecycle remains available for migration and audit. See [OpenClaw deployment](openclaw-host-lifecycle.md).
...
- **Beta Milestone 3:** **IMPLEMENTED for the disposable-fixture path.** Node 24's built-in SQLite owns transactional state, attempts, checkpoints, confirmations, leases, results, cursors, usage, artifact digests, and cleanup history. Explicit owner-confirmed resume continues only from mechanically proven safe evidence. Durable cleanup plans enforce host-owned age/byte policy while preserving minimum explanatory evidence. See the [Beta Milestone 3 report](beta-milestone-3.md).
- **Bounded child Git isolation:** writable child attempts receive host-derived branches and worktrees from immutable wave bases. Child commits remain proposals until the root performs an attributable integration transition; conflicts restore the root and preserve both sides. See [child proposal worktrees and root integration](child-git-integration.md).
- **Lossless child lifecycle:** cancellation intent, runtime inspection, complete subtree teardown, worker-death interruption, and resume bindings are durable and digest-bound. Bounded child evidence is protected by cleanup policy. See [child runtime lifecycle and recovery](child-runtime-lifecycle.md).
- **Beta Milestone 4:** **IMPLEMENTED and enabled by default for host-configured Prime jobs.** Native Prime child execution now composes durable admission, isolated proposal worktrees, immutable dependency waves, scoped inference leases, linked model-switch retries, root-routed reconnect controls, and trusted terminal gates. Set trusted host policy `multiChild` to `false` for the single-root fallback. A live selected-repository run completed the required implementation, test, and adversarial-review workflow. Broad rollout remains blocked by containment work. See the [Beta Milestone 4 report](beta-milestone-4.md).
...
Recommendation: select repositories explicitly and treat `unsafe-local` as a trusted-host execution mode. Runtime integrity is now enforced; filesystem, network, and process containment remains deferred.

See the [Beta Milestone 1 report](beta-milestone-1.md) for TDD evidence, live acceptance evidence, and remaining risks.
...
- Runtime artifacts are platform-, architecture-, and Node-binary-specific. Building and distributing them remains an operator release responsibility; artifact signing and remote provenance are not implemented.
...
Before broader adoption, make an explicit selected-repository rollout decision. Monetary cost reconciliation remains unavailable unless the subscription transport exposes a supported authoritative source. Container confinement remains a later hardening milestone. Smoke-test only a deliberately selected trusted local repository with a release-built runtime artifact.

See the [deep-review catalog](beta-milestone-1-review.md) for resolved findings and issue-ready deferred work.
