...
- **Beta Milestone 4:** **IMPLEMENTED behind an explicit experimental gate.** Native Prime child execution now composes durable admission, isolated proposal worktrees, immutable dependency waves, scoped inference leases, linked model-switch retries, root-routed reconnect controls, and trusted terminal gates. A live selected-repository run completed the required implementation, test, and adversarial-review workflow. Broad rollout remains blocked by runtime-integrity and containment work. See the [Beta Milestone 4 report](docs/beta-milestone-4.md).
