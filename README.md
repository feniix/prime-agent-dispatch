...
- **OpenClaw deployment:** native online and offline plugin archives install with `openclaw plugins install <artifact.tgz>`. The plugin derives the active profile, manages its runtime/state internally, and starts with a fail-closed empty repository policy. The earlier source lifecycle remains available for migration and audit. See [OpenClaw deployment](docs/openclaw-host-lifecycle.md).
